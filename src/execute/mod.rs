use crate::bus::Bus;
use crate::cpu::{
    AccessType, CAUSE_ECALL_M, CAUSE_ECALL_S, CAUSE_ECALL_U, CAUSE_ILLEGAL_INST,
    CAUSE_INST_MISALIGNED, CYCLE, CYCLEH, Hart, INSTRET, INSTRETH, MCOUNTEREN, MSTATUS,
    MSTATUS_TSR, MSTATUS_TVM, MSTATUS_TW, SATP, SCOUNTEREN, TIME, TIMEH,
};
...
pub fn execute(hart: &mut Hart, bus: &mut Bus, inst: Instruction) {
    match inst {
        ...
        Instruction::Lui { rd, imm } => {
            hart.write_reg(rd as usize, imm);
            hart.pc += 4;
        }
        Instruction::Auipc { rd, imm } => {
            hart.write_reg(rd as usize, hart.pc.wrapping_add(imm));
            hart.pc += 4;
        }
        ...
        Instruction::Csr { op, rd, rs1, csr } => {
            ...
            if csr == SATP && hart.privilege == 1 && (hart.read_csr(MSTATUS) & MSTATUS_TVM) != 0 {
                hart.trap(CAUSE_ILLEGAL_INST, 0);
                return;
            }
            // mcounteren/scounteren 控制计数器访问
            if matches!(csr, CYCLE | CYCLEH | TIME | TIMEH | INSTRET | INSTRETH)
                && hart.privilege < 3
            {
                let bit = if matches!(csr, CYCLE | CYCLEH) {
                    1u32 << 0
                } else if matches!(csr, TIME | TIMEH) {
                    1u32 << 1
                } else {
                    1u32 << 2
                };
            ...
            match op {
                CsrOp::Rw => {
                    hart.write_csr(csr, hart.read_reg(rs1 as usize));
                }
                CsrOp::Rs => {
                    if rs1 != 0 {
                        hart.write_csr(csr, old | hart.read_reg(rs1 as usize));
                    }
                }
                CsrOp::Rc => {
                    if rs1 != 0 {
                ...
                CsrOp::Rwi => {
                    hart.write_csr(csr, uimm);
                }
                CsrOp::Rsi => {
                    if rs1 != 0 {
                        hart.write_csr(csr, old | uimm);
                    }
                }
                CsrOp::Rci => {
                    if rs1 != 0 {
        ...
        Instruction::Amo {
            ...
            let access = if matches!(op, AmoOp::Lr) {
                AccessType::Read
            } else {
                AccessType::Write
            };
        ...
        // ===== System / Privilege =====
        Instruction::Ecall => {
            // M6：Linux 启动路径，S-mode ecall 直接交给内嵌 SBI 处理
            if hart.handle_sbi_ecall(bus) {
                hart.pc += 4;
                return;
            }
