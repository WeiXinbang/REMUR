...
fn run_test_return_cycles(bin_name: &str) -> u64 {
    ...
    assert_eq!(bus.tohost_value, Some(1),
        "{} FAIL: tohost = {:?}", bin_name, bus.tohost_value);
    cycles
}

fn bench_single_test(c: &mut Criterion, name: &str, bin_name: &str) {
    c.bench_with_input(BenchmarkId::new("riscv-test", name), &bin_name, |b, &bin| {
...
fn bench_all_tests(c: &mut Criterion) {
    let all_bins: Vec<&str> = vec![
        ...
        "rv32ui-p-ori", "rv32ui-p-sb", "rv32ui-p-sh",
        ...
        // rv32ua (10)
        "rv32ua-p-amoadd_w", "rv32ua-p-amoand_w", "rv32ua-p-amomax_w",
        "rv32ua-p-amomaxu_w", "rv32ua-p-amomin_w", "rv32ua-p-amominu_w",
        "rv32ua-p-amoor_w", "rv32ua-p-amoswap_w", "rv32ua-p-amoxor_w",
        "rv32ua-p-lrsc",
        // rv32mi (16)
        "rv32mi-p-breakpoint", "rv32mi-p-csr",
        "rv32mi-p-illegal", "rv32mi-p-instret_overflow",
        "rv32mi-p-lh-misaligned", "rv32mi-p-lw-misaligned",
        "rv32mi-p-ma_addr", "rv32mi-p-ma_fetch", "rv32mi-p-mcsr",
        "rv32mi-p-pmpaddr", "rv32mi-p-sbreak", "rv32mi-p-scall",
        "rv32mi-p-shamt", "rv32mi-p-sh-misaligned",
        "rv32mi-p-sw-misaligned", "rv32mi-p-zicntr",
        // rv32si (6)
        "rv32si-p-csr", "rv32si-p-dirty", "rv32si-p-ma_fetch",
