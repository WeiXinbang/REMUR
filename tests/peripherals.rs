...
fn clint_mtime_increments() {
    ...
    assert_eq!(result, Some(1), "CLINT mtime should increment between reads");
}
...
fn clint_timer_interrupt() {
    ...
    assert_eq!(result, Some(1), "Timer interrupt should fire and write tohost=1");
...
fn plic_pending_and_claim() {
    ...
    // pending 应已清除
    assert!(!bus.plic.has_pending_interrupt(), "pending should be cleared after claim");
